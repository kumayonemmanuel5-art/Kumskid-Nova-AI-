(function () {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      console.error('Nova AI chat error:', err.message);
      addMessage('bot', 'Sorry, I am having trouble right now. Please try again in a moment.');
    }
  ...
  async function fetchBotReply(userMessage) {
    ...
    const messages = [
      {
        role: 'system',
        content: window._novaBotPrompt ||
          `You are a helpful and friendly AI assistant for ${BOT_NAME}. You help customers with questions about products, services, pricing, delivery, and any other business-related questions. Always be professional, warm and helpful. If you do not know something specific, ask the customer to contact the business directly.`
      },
    ...
    // Handle both direct Groq response and wrapped response
    const reply = 
      data.choices?.[0]?.message?.content ||  // direct Groq format
      data.reply ||                             // wrapped format
      data.message ||                           // alternative format
      null;

    if (!reply) {
      console.error('Nova AI: unexpected response format', JSON.stringify(data).slice(0, 200));
