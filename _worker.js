...
export default {
  async fetch(request, env) {
    ...
    // ══════════════════════════════════════════════════════
    // ROUTE 1b: /gemini-vision — image-to-text for AI Tutor
    // Receives a base64 image, returns extracted question text
    // ══════════════════════════════════════════════════════
    if (path === '/gemini-vision') {
      if (method !== 'POST') {
        return new Response('OK - Gemini Vision endpoint is running', { status: 200 });
      }
      if (!env.GEMINI_API_KEY) {
        return corsResponse({ error: 'GEMINI_API_KEY not configured' }, 500);
      }
      try {
        ...
        if (geminiRes.status === 400 || geminiRes.status === 401 || geminiRes.status === 403) {
          return corsResponse({ error: 'Invalid Gemini API key.', code: 'INVALID_KEY' }, 401);
        }
